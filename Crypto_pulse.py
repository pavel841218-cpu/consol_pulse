...
MAX_PRICE: float = 10.0          # было 1.0 → теперь охватываем больше альтов
...
class ConsolidationMonitor:
    ...
    async def load_hourly_klines(self, symbol: str) -> Tuple[Optional[List[Dict[str, Any]]], Optional[float]]:
        ...
        if symbol in self.kline_cache:
            ...
            if age < 55:
                ...
                return cached["klines"], cached["ema40"]
        ...
        try:
            ...
            async with self.session.get(url, timeout=10) as resp:
                ...
                closed = klines[-41:-1]  # 40 закрытых свечей
                ...
                if symbol in self.ema_cache:
                    ...
                    if last_closed_candle_time == last_cached_time:
                        ...
                        ema40 = ema(closes, 40)
                        self.ema_cache[symbol] = (ema40, last_closed_candle_time)
                    else:
                        k = 2 / (40 + 1)
                        new_close = closes[-1]
                        ema40 = new_close * k + prev_ema * (1 - k)
                        self.ema_cache[symbol] = (ema40, last_closed_candle_time)
                else:
                    ema40 = ema(closes, 40)
                    self.ema_cache[symbol] = (ema40, last_closed_candle_time)
    ...
    async def check_consolidation_and_pump(self, target_data: Dict[str, Any]) -> None:
        ...
        async with self.semaphore:
            ...
            try:
                klines, ema40 = await asyncio.wait_for(
            ...
            if klines is None or ema40 is None:
                return
            ...
            # 1. Фильтр тренда (EMA40)
            if current_close < ema40:
                self.increment_stat("filtered_by_trend")
                logger.debug(f"{symbol} отклонён: цена ниже EMA40 ({current_close:.6f} < {ema40:.6f})")
                return
            ...
            for idx in range(len(history) - 1, -1, -1):
                ...
                # ----- ОТКЛЮЧЁН ВНУТРЕННИЙ ATR-ФИЛЬТР (чтобы база не рвалась) -----
                # if atr_base_pct >= 1.5:
                #     break
            ...
            # ==== ИСПОЛЬЗУЕМ current_close ДЛЯ ПРОБОЯ (более актуально) ====
            breakout = ((current_close - highest_base) / highest_base) * 100
            ...
            # 9. Рост внутри свечи (ослаблен до 0.2%)
            if growth < 0.2:
                self.increment_stat("filtered_by_breakout")
                logger.debug(f"{symbol} отклонён: рост {growth:.2f}% < 0.2%")
                return

            # ----- УБРАНЫ ДОПОЛНИТЕЛЬНЫЕ ФИЛЬТРЫ ОБЪЁМА -----
