...
# ============================================================
#  БОТ НА ЛОНГ ОТ БАЗЫ (ЧИСТЫЙ ПРОБОЙ ВВЕРХ)
# ============================================================
...
# --- Настройки стратегии ---
BASE_CANDLES_COUNT = 4          # Размер базы (4 последние закрытые свечи)
MIN_BREAKOUT_PCT = 1.5          # Пробой верха базы на +1.5% и выше
MIN_24H_VOLUME_USDT = 30_000    # Мин. ликвидность
...
async def check_symbol(session, symbol, vol_24h, sent_alerts, semaphore):
    ...
    # Закрытые свечи базы (например, последние 4 часа)
    base_candles = candles[-(BASE_CANDLES_COUNT + 1):-1]
    current_candle = candles[-1]

    # Находим абсолютный максимум базы (верх полки)
    base_high = max(c["high"] for c in base_candles)
    if base_high <= 0: return

    current_price = current_candle["close"]

    # Ищем СТРОГО ЛОНГ: пробой верха базы вверх
    ...
    message = (
        ...
        f"├ Верх базы (хай 4ч): <code>{format_price(base_high)}</code>\n"
