...
# ============================================================
# 🏹 ПАРТИЗАН v6.5.1 — STRICT REAL-TIME BREAKOUT & ANTI-MM
# ============================================================
...
def evaluate_first_impulse(candles, shelf):
    ...
    # Первая свеча после полки должна закрываться выше верха полки
...
def is_market_maker_noise(breakout_candle, open_interest=None):
    ...
    # 1. Слишком малое тело свечи (доджи)
...
def find_best_shelf(candles):
    ...
    # Ищем полки, которые заканчиваются ТОЛЬКО на последних 1-3 свечах истории!
    ...
    # Приоритет отдаем самым свежим полкам
...
async def evaluate_symbol(session, symbol, ticker):
    try:
        ...
        # Исключаем текущую еще не сформированную свечу
        closed = candles[:-1]
        if len(closed) < 80:
            return None
        ...
        # ПРОВЕРКА НА МАРКЕТ-МЕЙКЕРА И СКВИЗЫ
...
async def main():
    logger.info("🏹 Запуск ПАРТИЗАН v6.5 (Strict Breakout Fix)...")
    app = web.Application()
