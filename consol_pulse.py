...
# ===== НАСТРОЙКИ СЖАТИЯ EMA И ВОЛАТИЛЬНОСТИ =====
...
async def health_check(request):
    return web.Response(text="Partizan Bot (EMA20/40/80) Active", status=200)
...
def check_1h_ema_squeeze(candles_1h):
    """
    Анализ 1H:
    - Сжатие EMA20/EMA40/EMA80
    - Плотность цен закрытия (Close)
    - Затухание волатильности перед выстрелом
    """
    if len(candles_1h) < 90:
        return False, 0, 0, 0

    closes = [c["close"] for c in candles_1h[:-1]] # Только полностью закрытые 1H свечи

    # 1. Расчет EMA 20, 40, 80
    ...
    # 2. Оценка плотности закрытий (Close) за последние 6 часов
    ...
    # 3. Затухание волатильности на последней закрытой 1H свече перед стартом
    ...
    # Определение уровней базы по свечам флэта
    recent_candles = candles_1h[-7:-1]
    base_high = max(c["high"] for c in recent_candles)
    base_low = min(c["low"] for c in recent_candles)
...
async def main():
    ...
    try:
        await bot.send_message(chat_id=CHAT_ID, text="🤖 Бот «ПАРТИЗАН» запущен:\nФильтр: EMA 20/40/80 Squeeze + 5M Breakout!")
    except Exception as e:
