...
# ============================================================
#                    SHELF SETTINGS (ИСПРАВЛЕНО)
# ============================================================

# Теперь ищем базы от 4 до 12 часовых свечей
MIN_SHELF_CANDLES = 4
MAX_SHELF_CANDLES = 12

# Более мягкие ограничения ширины
MAX_SHELF_WIDTH_PCT = 10.0          # было 7.0
MAX_SHELF_WICK_WIDTH_PCT = 12.0     # было 9.0

# EMA спред стал шире
EMA_FAST = 20
EMA_SLOW = 40
EMA_MAX_SPREAD_PCT = 8.0            # было 6.0
...
async def update_futures_symbols(session):
    ...
    try:
        async with session.get(url, headers=headers, timeout=10) as resp:
            ...
            symbols = {
                s for s in symbols if s and is_crypto_usdt_symbol(s)
            }
...
async def get_klines(session, symbol, interval="1h", limit=60):
    ...
    try:
        async with session.get(url, params=params, headers=headers, timeout=8) as resp:
            if resp.status != 200:
                logging.debug("⚠️ BingX HTTP Error %s: status %d", symbol, resp.status)
                return []

            data = await resp.json()
            if data.get("code") != 0:
                logging.debug("⚠️ BingX Code Error %s: %s", symbol, data.get("msg"))
                return []
...
def check_shelf_before_impulse(candles):
    ...
    # Для нормального EMA40 нужна достаточная история
    ...
    # ИЩЕМ ПОЛКУ В РАЗНЫХ ОКНАХ (теперь 4-12)
    for window in range(MIN_SHELF_CANDLES, MAX_SHELF_CANDLES + 1):
        if window > total:
            continue

        # Расширенное окно поиска конца полки: до 20 свечей назад
    ...
    for sym in (symbol, symbol.replace("-", "")):
        try:
            async with session.get(url, params={"symbol": sym}, headers=headers, timeout=5) as resp:
                ...
                if value < 500_000 and current_price > 0:
...
async def scan_one_symbol_for_shelf(session, ticker):
    ...
    candles_raw = await get_klines(session, symbol, "1h", 60)
...
# ============================================================
#                    FULL MARKET SCAN (RATE-LIMIT SAFE)
# ============================================================
...
async def full_market_scan(session):
    ...
    # Ограничение до 5 одновременных запросов для защиты от спам-фильтра BingX
    ...
    async def worker(ticker):
        async with semaphore:
            try:
                ...
                await asyncio.sleep(0.05)  # Небольшая пауза между запросами (50 мс)
                return res
            except Exception as e:
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        while True:
            ...
            # Проверка необходимости полного сканирования
