...
async def full_market_scan(session):
    ...
    while not tickers and retry_count < 3:
        retry_count += 1
        logging.warning("⚠️ Тикеры не получены. Повтор %d/3 через 5 сек...", retry_count)
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        while True:
            ...
            while not tickers and retry_count < 3:
                retry_count += 1
                logging.warning("⚠️ Тикеры не получены. Повторная попытка %d/3 через 5 сек...", retry_count)
                await asyncio.sleep(5)
                tickers = await get_market_tickers(session)
