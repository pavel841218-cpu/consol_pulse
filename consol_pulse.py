...
# Основной эндпоинт фьючерсов
BASE_URL = "https://fapi.binance.com"

# Настройки Webshare прокси
...
# ============================================================
#            BINANCE API HELPERS (ЧЕРЕЗ PROXY)
# ============================================================
...
async def get_klines(session, symbol, interval="1h", limit=100):
    ...
    try:
        async with session.get(url, params=params, proxy=PROXY_URL, timeout=8) as resp:
            if resp.status != 200:
                return []
...
async def fetch_cvd_metrics(session, symbol):
    ...
    try:
        async with session.get(url, params=params, proxy=PROXY_URL, timeout=4) as resp:
            if resp.status == 200:
...
async def check_book_spread(session, symbol, max_allowed_spread_pct):
    ...
    try:
        async with session.get(url, params=params, proxy=PROXY_URL, timeout=3) as resp:
            if resp.status == 200:
                data = await resp.json()
...
async def fetch_current_open_interest(session, symbol, price):
    ...
    try:
        async with session.get(url, params={"symbol": symbol}, proxy=PROXY_URL, timeout=5) as resp:
            if resp.status != 200:
                return None
            data = await resp.json()
...
async def fetch_oi_growth(session, symbol):
    ...
    try:
        async with session.get(url, params=params, proxy=PROXY_URL, timeout=5) as resp:
            if resp.status != 200:
                return None, None
            data = await resp.json()
            if len(data) < 2:
...
async def is_asset_alive(session, symbol) -> tuple[bool, str]:
    ...
    try:
        async with session.get(klines_url, params={"symbol": symbol, "interval": "1d", "limit": 30}, proxy=PROXY_URL, timeout=4) as resp:
            if resp.status == 200:
...
async def analyze_symbol(session, symbol, price, quote_volume, change_24h, semaphore):
    async with semaphore:
        try:
            ...
            # 2. Проверка базовых свечей для полки
            ...
            # 3. Тяжелые фильтры (запрашиваются ТОЛЬКО если есть пробой полки)
...
async def send_telegram_alert(session, text):
    ...
    try:
        # Без прокси для максимальной скорости доставки
