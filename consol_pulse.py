...
# ============================================================
#  БОТ «ПАРТИЗАН» (ЖЕСТКИЙ ФИЛЬТР: ПОЛКА + EMA + ИСТОРИЯ ОИ + ИМПУЛЬС)
# ============================================================
...
MAX_SHELF_WIDTH_PCT = 2.5       # Сузили полку до 2.5% (только узкие базы)
MIN_BREAKOUT_PCT = 3.0          # Пробой от +3.0% (отсекает мелких скальперов)
MIN_OI_GROWTH_PCT = 2.5         # ОИ должен вырасти минимум на +2.5% по истории
MIN_24H_VOLUME_USDT = 3_000_000 # Мин. объем $3M
...
async def get_oi_growth(session, symbol, semaphore):
    """ Проверка динамики ОИ по истории часовых свечей BingX """
    url = f"{BINGX_BASE}/openApi/swap/v2/quote/openInterestHistory"
...
async def check_symbol(session, symbol, vol_24h, sent_alerts, semaphore):
    ...
    # 4. Проверка заноса денег через Историю ОИ (минимум +2.5%)
    oi_growth_pct = await get_oi_growth(session, symbol, semaphore)
    if oi_growth_pct < MIN_OI_GROWTH_PCT: return

    clean_coin = symbol.split("-")[0].upper()
    message = (
        f"🚀 <b>ИМПУЛЬС С ДЕНЬГАМИ</b>\n\n"
        f"Монета: <code>{clean_coin}</code>\n"
        f"💥 Пробой: <b>+{breakout_pct:.2f}%</b>\n"
        f"📈 Прирост ОИ (1H): <b>+{oi_growth_pct:.2f}%</b>\n"
    ...
    logging.info(f"СИГНАЛ: {clean_coin} | Пробой: +{breakout_pct:.2f}% | ОИ: +{oi_growth_pct:.2f}%")

async def main():
    sent_alerts = {}
    semaphore = asyncio.Semaphore(10)
    async with aiohttp.ClientSession() as session:
        logging.info("Скрипт перезапущен с фильтром ОИ")
        await send_telegram(session, "🛡 <b>Бот Партизан обновлен!</b> Фильтр ОИ +3.0% импульс запущен.")
