...
MIN_24H_VOLUME_USDT = 1_500_000 # Мин. суточный объем $1.5M (для средних монет)
...
async def get_oi_growth(session, symbol, semaphore):
    ...
    async with semaphore:
        # 1. Проверяем Binance
        try:
            async with session.get(binance_url, params=params_binance, headers=headers, timeout=aiohttp.ClientTimeout(total=3)) as resp:
                if resp.status == 200:
                    data = await resp.json()
        ...
        # 2. Переключаемся на BingX, если монеты нет на Binance
        try:
