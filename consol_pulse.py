...
def get_adaptive_thresholds(price: float) -> dict:
    """
    Возвращает пороги в зависимости от ценового диапазона.
    Порог максимального отхода от полки (max_breakout_pct) динамически выше минимального!
    """
    if price < 0.0005:
        return {
            "tier_name": "ULTRA",
...
async def fetch_rvol_4h(session, symbol):
    ...
    if len(klines) < 10:
        return 1.0
    # Используем Quote Volume напрямую из klines[7]
...
async def analyze_symbol(session, symbol, price, quote_volume, change_24h, semaphore):
    async with semaphore:
        try:
            ...
            if quote_volume < t["min_volume_24h"]:
                return
            ...
            # Исправленная проверка выхода цены
            if price_change_pct < t["min_breakout_pct"]:
                return

            if price_change_pct > t["max_breakout_pct"]:
                return
