...
class ConsolidationMonitor:
    ...
    async def get_active_symbols(self) -> List[Dict[str, Any]]:
        ...
        try:
            async with self.session.get(f"{BINGX_API_URL}/contracts", timeout=15) as resp:
                if resp.status != 200:
                    return self.active_symbols_cache  
                data = await resp.json()
            ...
            async with self.session.get(url, timeout=10) as resp:
                ...
                if symbol in self.ema_cache:
                    ...
                    if last_closed_candle_time == last_cached_time:
                        ...
                        # Пропущено больше одной свечи — полный перерасчет (Слайд 2)
    ...
    async def check_consolidation_and_pump(self, target_data: Dict[str, Any]) -> None:
        ...
        async with self.semaphore:
            ...
            if current_close < ema200:
                ...
                return  # Убран агрессивный stop_monitoring() (Слайд 6)
            ...
            for idx in range(len(history) - 1, -1, -1):
                ...
                if new_lowest <= 0:
                    return  # Убран агрессивный stop_monitoring()
            ...
            if EARLY_SIGNAL:
                ...
                # Точный нелинейный расчет прогресса часа (Слайд 3)
            ...
            if USE_OPEN_INTEREST:
                # Прямой актуальный запрос ОИ непосредственно перед сигналом (Слайд 1)
            ...
            last_base_candle_time = int(base[-1]["time"])  # Время закрытия последней свечи базы
            
            if symbol in BREAKOUT_MEMORY:
                stored_base_candle_time, last_time = BREAKOUT_MEMORY[symbol]
                # Сверяемся по времени свечи базы, а не по её абстрактной высоте (Слайд 4)
    ...
    async def start_loop(self) -> None:
        ...
        while True:
            try:
                ...
                # Вызываем cleanup чаще — каждые 15 циклов (~15 минут) для поддержания чистоты (Слайд 10)
                if self._iteration % 15 == 0:
                    self.cleanup()

                # Если API лежит, используем старый кэш контрактов, но продолжаем проверки (Слайды 8, 9)
    ...
    try:
        ...
        if "callback_query" in data:
            ...
            if cb_data.startswith("block_"):
                ...
                # Сброс кеша активных тикеров, чтобы моментально исключить заблокированный токен
