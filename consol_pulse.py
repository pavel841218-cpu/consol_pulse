...
SHELF_TTL = 12 * 60 * 60          # полка живёт макс 12 часов
TRIGGERED_TTL = 30 * 60           # после сигнала держим ещё 30 минут
...
# ============================================================
#                    SHELF SETTINGS (СТРОГИЕ ФИЛЬТРЫ)
# ============================================================

MIN_SHELF_CANDLES = 6             # Минимум 6 часов
MAX_SHELF_CANDLES = 36            # Максимум 36 часов

MAX_SHELF_WIDTH_PCT = 2.5         # Тело полки строго <= 2.5%
MAX_SHELF_WICK_WIDTH_PCT = 3.0    # Полный размах с тенями <= 3.0%

EMA_FAST = 20
EMA_SLOW = 40
EMA_MAX_SPREAD_PCT = 1.2          # Спред EMA20/40 <= 1.2%
...
# ============================================================
#                  NON-CRYPTO & STABLE FILTER
# ============================================================

# Расширенный список традиционных активов / акций / сырья
EXCLUDED_BASES = {
    "USDC", "FDUSD", "USD1", "TUSD", "BUSD", "DAI", "EUR", "GBP", "JPY", "AUD",
    "BZ", "NATGAS", "COPPER", "GOLD", "SILVER", "BRENT", "WTI", "OIL",
    "SPY", "QQQ", "SOX", "TQQQ", "NVDA", "PLTR", "TSLA", "AAPL", "MSFT", "AMZN",
    "META", "GOOG", "INTC", "NFLX", "COIN", "MSTR", "IREN", "LITE", "SAMSUNG",
...
def is_crypto_usdt_symbol(symbol):
    ...
    # Исключение тикеров с цифрами/спецсимволами в начале (индексы)
    if not re.match(r"^[A-Z0-9]{2,12}$", base):
        return False

    for item in EXCLUDED_BASES:
        if base.startswith(item) or base.endswith(item):
            if len(base) > len(item) and not base.startswith("BTC") and not base.startswith("ETH"):
                return False
...
def load_shelves():
    ...
    try:
        ...
        if isinstance(data, dict):
            valid_shelves = {}
            for sym, shelf in data.items():
                if not is_crypto_usdt_symbol(sym):
                    continue
                
                width = safe_float(shelf.get("width"), 999.0)
                body_width = safe_float(shelf.get("body_width"), 999.0)

                if width <= MAX_SHELF_WICK_WIDTH_PCT and body_width <= MAX_SHELF_WIDTH_PCT:
                    valid_shelves[sym] = shelf
            
            SHELVES = valid_shelves
            logging.info("💾 Загружено и отфильтровано сохранённых полок: %d", len(SHELVES))
        else:
            SHELVES = {}
    except Exception as e:
    ...
    try:
        async with session.get(url, headers=HEADERS, timeout=10) as resp:
            ...
            if symbols:
                ...
                logging.info("✅ Крипто-фьючерсов Binance загружено: %d", len(symbols))
...
async def send_startup_message(session):
    text = (
        ...
        f"🧲 Макс. тени полки: <b>{MAX_SHELF_WICK_WIDTH_PCT:.1f}%</b>\n"
...
def check_shelf_before_impulse(candles):
    ...
    for end_idx in [total - 1, total - 2]:
        if end_idx < MIN_SHELF_CANDLES:
            continue
        ...
        for window in range(MIN_SHELF_CANDLES, MAX_SHELF_CANDLES + 1):
            ...
            base_opens = [c["open"] for c in base if c["open"] > 0]
            ...
            shelf_slope = (abs(last_close - first_close) / first_close) * 100
            if shelf_slope > 1.0:
                continue
            ...
            # 4. Проверка зажима EMA20 и EMA40
            base_ema20 = ema20_series[start_idx:end_idx + 1]
            base_ema40 = ema40_series[start_idx:end_idx + 1]

            if any(e20 <= 0 or e40 <= 0 for e20, e40 in zip(base_ema20, base_ema40)):
                continue

            max_ema_spread = max((abs(e20 - e40) / e40) * 100 for e20, e40 in zip(base_ema20, base_ema40))
            if max_ema_spread > EMA_MAX_SPREAD_PCT:
                continue

            # 5. Фильтр шума
            candle_ranges = [((c["high"] - c["low"]) / c["low"]) * 100 for c in base if c["low"] > 0]
            if not candle_ranges:
                continue

            quiet_count = sum(1 for r in candle_ranges if r <= 2.0)
            ...
            score = (
                min(window, 16) * 0.20 +
                max(0, 2.5 - body_width) +
                max(0, 1.2 - max_ema_spread) +
                quiet_ratio * 3
            )
...
async def full_market_scan(session):
    ...
    if not tickers:
        logging.error("❌ Не удалось получить тикеры для полного скана.")
        return
    ...
    for shelf in results:
        ...
        if symbol in SHELVES:
            ...
            SHELVES[symbol]["low"] = shelf["low"]
            SHELVES[symbol]["high"] = shelf["high"]
            SHELVES[symbol]["width"] = shelf["width"]
            SHELVES[symbol]["body_width"] = shelf["body_width"]
            updated += 1
        else:
            SHELVES[symbol] = shelf
            found += 1
            logging.info(
                "🧲 НОВАЯ ПОЛКА | %s | %.8f - %.8f | тело %.2f%% | тени %.2f%% | свечей=%d",
                symbol,
                shelf["low"],
                shelf["high"],
                shelf["body_width"],
...
async def check_shelf_impulse(session, shelf, ticker):
    ...
    message = (
        ...
        f"📐 Тени: <b>{shelf.get('width', 0):.2f}%</b>\n"
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        while True:
            ...
            retry_count = 0
            while not tickers and retry_count < 4:
                retry_count += 1
            ...
            if tickers:
                ...
                logging.error("❌ Не удалось получить тикеры после повторов.")
