...
WATCH_INTERVAL = 20            # 20 секунд при наличии полок
    ...
    try:
        async with session.get(url, headers=get_headers(), timeout=10) as resp:
            if resp.status in (418, 429):
                logging.warning(f"⚠️ Binance Rate Limit (HTTP {resp.status})! Засыпаем на 120 сек...")
...
async def analyze_symbol_for_shelf(session, sym, semaphore):
...
async def scan_market(session, tickers):
    ...
    tasks = [analyze_symbol_for_shelf(session, sym, semaphore) for sym in tickers.keys()]
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            try:
                ...
                if tickers:
                    await scan_market(session, tickers)

                # 2. Если полок НЕТ — просто спим полные 15 минут без одиночных запросов
                if not ACTIVE_SHELVES:
                    logging.info(f"💤 Активных полок нет. Режим сна на 15 минут...")
                    await asyncio.sleep(FULL_SCAN_INTERVAL)
                    continue

                # 3. Если полки ЕСТЬ — отслеживаем их в течение 15 минут каждые 20 сек
                start_watch_time = now_ts()
                while (now_ts() - start_watch_time) < FULL_SCAN_INTERVAL:
                    if not ACTIVE_SHELVES:
                        break
