...
OI_HISTORY = {}  # Для отслеживания изменений OI
...
def check_liquidity_shelf(candles_1h, current_price, oi_usdt, change_24h, rvol_5m):
    ...
    Возвращает (bool, str) - статус прохождения и причину.
    """
    try:
        # 1. Фильтр ОИ: работаем только с крупным капиталом
        MIN_OI = MIN_OPEN_INTEREST_USDT
        if oi_usdt < MIN_OI:
            return False, f"Низкий ОИ (${oi_usdt/1e6:.1f}M < ${MIN_OI/1e6:.0f}M)"

        # 2. Фильтр суточного роста: отгоняем уже перегретые ракеты
        MAX_24H_CHANGE = 15.0
        if change_24h > MAX_24H_CHANGE:
            return False, f"Монета уже улетела (+{change_24h:.1f}% за 24ч)"
        ...
        # Отклонение текущей цены от EMA 20
        price_to_ema20_diff = abs(current_price - ema20) / ema20 * 100
        # Дистанция между самими EMA
        ...
        # 4. Фильтр всплеска: краткосрочный объем только начинает вливаться
        MIN_RVOL_5M = 1.5
        if rvol_5m < MIN_RVOL_5M:
            return False, f"Слаб краткосрочный объем (RVOL 5m: {rvol_5m:.2f}x)"

        return True, f"🎯 ПОЛКА СФОРМИРОВАНА | Дистанция от EMA20: {price_to_ema20_diff:.1f}%"
...
async def send_startup_message(session):
    message = (
        "🟢 <b>Crypto_pulse запущен (Полка Ликвидности + OI)</b>\n\n"
        ...
        f"🧲 Полка: <b>EMA20/40 сжатие + RVOL 5m ≥ 1.5</b>\n\n"
...
async def process_tickers(session, tickers):
    ...
    for (symbol, current_price, quote_volume) in tickers:
        ...
        # 3. ПОЛУЧЕНИЕ ТЕКУЩЕГО ОИ И ПРОВЕРКА ПОРОГА
        ...
        # 3.5 ПОЛУЧЕНИЕ СВЕЧЕЙ ДЛЯ ПОЛКИ ЛИКВИДНОСТИ
        ...
        # 3.6 ПРОВЕРКА ПОЛКИ ЛИКВИДНОСТИ
        ...
        alerts += 1
        logging.info(
            "🚀 СИГНАЛ | %s | %s%.2f%% за %dm | RVOL 1H %.2fx | Short RVOL %.2fx | OI $%.0f | %s",
            clean_coin, sign, change_pct, actual_minutes, hourly_rvol, short_rvol, oi_usdt, shelf_reason
        )
...
async def main_loop():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        logging.info("🚀 CRYPTO_PULSE ЗАПУЩЕН (Полка Ликвидности + OI)")
