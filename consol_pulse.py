...
# Полосы Боллинджера (информационно)
BB_PERIOD: int = 20
BB_STD: float = 2.0
...
def calculate_bollinger(closes: List[float], period: int = 20, std_dev: float = 2.0) -> Tuple[float, float, float]:
    """Возвращает (lower, middle, upper) полосы Боллинджера"""
...
class ConsolidationMonitor:
    def __init__(self) -> None:
        ...
        self.active_symbols_cache_ttl: float = 75.0
        
        # Кэш для EMA40 и Боллинджера
        ...
        self.stats_all_time: Dict[str, int] = {
            ...
            "filtered_by_overextended": 0, "filtered_by_prev_bullish": 0,
            "filtered_by_close_position": 0
        }
                ...
                if self.session and not self.session.closed:
                    async with self.session.get(f"{BINGX_API_URL}/contracts", timeout=10) as resp:
                        if resp.status == 200:
                            ...
                            logger.warning("API ping: 429 Too Many Requests.")
                        elif resp.status in (500, 503):
                            logger.error(f"API ping: Server Error {resp.status}.")
    ...
    async def load_hourly_klines(self, symbol: str) -> Tuple[Optional[List[Dict[str, Any]]], Optional[Dict[str, float]]]:
        """Загружает свечи и возвращает индикаторы (EMA40, Bollinger)"""
        if not self.session or self.session.closed:
            return None, None
        ...
        if symbol in self.kline_cache:
            ...
            if age < 55:
                ...
                return cached["klines"], cached["indicators"]
        ...
        try:
            ...
            async with self.session.get(url, timeout=10) as resp:
                ...
                # Закрытые свечи (без текущей незакрытой)
                closed = klines[-42:-1]  # 41 свеча
                closes = [float(x["close"]) for x in closed]
                last_closed_candle_time = int(closed[-1]["time"])
                
                # EMA40
                ema40_curr = ema(closes, 40)
                ema40_prev = ema(closes[:-1], 40) if len(closes) > 40 else ema40_curr
                
                # Bollinger Bands (по закрытым свечам)
                bb_lower, bb_middle, bb_upper = calculate_bollinger(closes, BB_PERIOD, BB_STD)
                
                indicators = {
                    "ema40_prev": ema40_prev,
                    "ema40_curr": ema40_curr,
                    "bb_lower": bb_lower,
                    "bb_middle": bb_middle,
                    "bb_upper": bb_upper,
                    "last_closed_time": last_closed_candle_time
                }
    ...
    async def check_consolidation_and_pump(self, target_data: Dict[str, Any]) -> None:
        ...
        async with self.semaphore:
            ...
            try:
                klines, indicators = await asyncio.wait_for(
            ...
            if klines is None or indicators is None:
                return
            ...
            ema40_prev = indicators["ema40_prev"]
            ema40_curr = indicators["ema40_curr"]
            bb_lower = indicators["bb_lower"]
            bb_middle = indicators["bb_middle"]
            bb_upper = indicators["bb_upper"]

            # ===== EMA и Bollinger — чисто информационные индикаторы =====
            # EMA статус
            if ema40_curr > ema40_prev and current_close > ema40_curr:
                ema_status = "✅ выше (тренд ↑)"
            elif ema40_curr > ema40_prev:
                ema_status = "⚠️ выше (но цена ниже)"
            elif current_close > ema40_curr:
                ema_status = "⚠️ ниже (но цена выше)"
            else:
                ema_status = "❌ ниже (тренд ↓)"
            ...
            if current_close <= bb_lower:
                ...
                bb_status = "🟡 выше средней"
            else:
                bb_status = "🟠 ниже средней"
            ...
            for idx in range(len(history) - 1, -1, -1):
                ...
                # ----- ОТКЛЮЧЁН ВНУТРЕННИЙ ATR-ФИЛЬТР -----
                # if atr_base_pct >= 1.5:
                #     break
            ...
            period = len(base)
            # 2. Фильтр длины базы
            if period < 3 or period > 48:
                self.increment_stat("filtered_by_breakout")
                logger.debug(f"{symbol} отклонён: длина базы {period} (допустимо 3-48)")
                return
            ...
            # ==== ЛОГИКА КРИПТО-НОЖКИ: ВХОД ОТ НИЖНЕЙ ГРАНИЦЫ ====
            lowest_base = min(float(x["low"]) for x in base)
            ...
            # Проверка: предыдущая свеча касалась нижней границы (зона ликвидности)
            distance_from_low = ((prev_low - lowest_base) / lowest_base) * 100
            if distance_from_low > 2.0:
                self.increment_stat("filtered_by_breakout")
                logger.debug(f"{symbol} отклонён: предыдущая свеча не коснулась нижней границы (dist={distance_from_low:.2f}%)")
                return

            # Проверка: предыдущая свеча не пробила нижнюю границу сильно вниз
            if prev_close < lowest_base * 0.98:
                self.increment_stat("filtered_by_breakout")
                logger.debug(f"{symbol} отклонён: предыдущая свеча пробила нижнюю границу вниз")
                return

            # Проверка: текущая свеча — бычья (зелёная)
            if current_close <= current_open:
                self.increment_stat("filtered_by_breakout")
                logger.debug(f"{symbol} отклонён: текущая свеча не бычья")
                return

            # Отскок от нижней границы (bounce)
            bounce = ((current_close - lowest_base) / lowest_base) * 100
            if bounce < MIN_BOUNCE_PCT:
                self.increment_stat("filtered_by_breakout")
                logger.debug(f"{symbol} отклонён: отскок {bounce:.2f}% < {MIN_BOUNCE_PCT}%")
                return

            # Закрытие текущей свечи в верхней половине диапазона (уверенный отскок)
            close_position = (current_close - current_low) / (current_high - current_low) if (current_high - current_low) > 0 else 0
            if close_position < 0.5:
                self.increment_stat("filtered_by_close_position")
                logger.debug(f"{symbol} отклонён: закрытие в нижней части свечи ({close_position:.2f})")
                return

            # ==== КОНЕЦ ЛОГИКИ КРИПТО-НОЖКИ ====
            ...
            if growth < 0.2:
                self.increment_stat("filtered_by_breakout")
                logger.debug(f"{symbol} отклонён: рост {growth:.2f}% < 0.2%")
                return
            ...
            if USE_OPEN_INTEREST:
                ...
                if fresh_oi is not None and len(history_oi) >= OI_MIN_HISTORY:
                    ...
                    if len(history_oi) >= 1 and fresh_oi <= history_oi[-1]:
                        ...
                        logger.debug(f"{symbol} отклонён: OI не вырос")
                        return
                    ...
                    if average_oi > 0:
                        ...
                        if oi_growth < MIN_OI_GROWTH:
                            self.increment_stat("filtered_by_oi")
                            logger.debug(f"{symbol} отклонён: рост OI {oi_growth:.2f}%")
                            return
            ...
            clean_esc = escape_markdown(clean)
            current_close_esc = escape_markdown(str(current_close))
            
            signal_type_esc = escape_markdown(signal_type)
            bounce_esc = escape_markdown(f"{bounce:.2f}%")
            ...
            ema_status_esc = escape_markdown(ema_status)
            bb_status_esc = escape_markdown(bb_status)
...
async def handle_tg_webhook(request: web.Request) -> web.Response:
    try:
        ...
        if "message" in data:
            ...
            if text == "/status":
                status_text = (
                    f"📊 *Consol Pulse Pro — КРИПТО-НОЖКА*\n\n"
                    f"• BB период: *{BB_PERIOD}*\n"
                    f"• Compression: *≤ {escape_markdown(f'{MAX_CONSOLIDATION_RANGE}%')}*\n"
                    f"• Base ATR: *отключён*\n"
                    f"• Bounce: *≥ {escape_markdown(f'{MIN_BOUNCE_PCT}%')}*\n"
