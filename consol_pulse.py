...
MAX_SHELF_WIDTH_PCT = 3.5         # тело полки не шире 3.5%
MAX_SHELF_WICK_WIDTH_PCT = 12.0   # с тенями не шире 12%
...
def check_shelf_before_impulse(candles):
    ...
    # ФИЛЬТР СВЕЖЕСТИ: Полка должна заканчиваться строго на последней или предпоследней закрытой свече!
    for end_idx in [total - 1, total - 2]:
        ...
        for window in range(MIN_SHELF_CANDLES, MAX_SHELF_CANDLES + 1):
            ...
            # 1. Проверка полной ширины с тенями
            ...
            # 2. Проверка ширины тел
            body_width = ((close_high - close_low) / close_low) * 100
            if body_width > MAX_SHELF_WIDTH_PCT:
                continue

            # 3. ФИЛЬТР НАКЛОНА ПОЛКИ: первая и последняя свечи должны быть на одном уровне!
            ...
            # 4. Проверка спреда и наклона EMA
            ...
            ema_start = ema20_series[start_idx]
            ema_slope = (abs(ema20 - ema_start) / ema_start) * 100
            if ema_slope > 1.2:  # Наклон EMA20 не должен превышать 1.2%
                continue
            ...
            quiet_count = sum(1 for r in candle_ranges if r <= 4.0)
            ...
            score = (
                min(window, 16) * 0.20 +
                max(0, 4.0 - body_width) +
                max(0, 4.0 - ema_spread) +
                quiet_ratio * 3
            )
...
async def check_shelf_impulse(session, shelf, ticker):
    ...
    # Защита от запоздавших входов: если цена уже улетела слишком далеко (> 5%), игнорируем!
    if movement > 5.0:
        shelf["last_price"] = price
        return
