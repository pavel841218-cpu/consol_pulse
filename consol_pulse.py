...
BASE_CANDLES_COUNT = 4          # База из 4 свечей по 1H (4 часа)
MAX_SHELF_WIDTH_PCT = 4.5       # Максимальная ширина полки (4.5%)
MIN_BREAKOUT_PCT = 1.8          # Пробой верхней границы от +1.8%
MIN_24H_VOLUME_USDT = 1_200_000 # Мин. суточный объем $1.2M

CHECK_INTERVAL_SECONDS = 30     
ALERT_COOLDOWN_SECONDS = 4 * 3600 # Кулдаун 4 часа на повторный сигнал
...
async def get_oi_growth(session, symbol, semaphore):
    """Быстрый и надежный запрос Открытого Интереса (ОИ) через Bybit API без блокировок Render"""
...
async def check_symbol(session, bot, symbol, vol_24h, semaphore):
    ...
    # 1. Формирование базы из 4 свечей перед текущей
    ...
    # Проверка поджатия: EMA20 зажата в базе (с допуском 0.2%)
    ema_in_base_count = 0
    for i in range(-5, -1):
        e20 = ema20_list[i]
        if (base_low * 0.998) <= e20 <= (base_high * 1.002):
            ema_in_base_count += 1

    # ТРЕБОВАНИЕ: EMA20 внутри базы на 3 или 4 свечах (>= 3)
    ...
    # Получение ОИ через Bybit
    oi_growth_pct, oi_source = await get_oi_growth(session, symbol, semaphore)

    last_signals[symbol] = now
    success = await send_signal(
        bot, symbol, breakout_pct, oi_growth_pct, 
        base_high, current_price, shelf_width_pct, vol_24h, oi_source
    )
    
    if success:
        logging.info(f"СИГНАЛ: {symbol} | Пробой: +{breakout_pct:.2f}% | Полка: {shelf_width_pct:.2f}% | EMA свечей: {ema_in_base_count}/4 | ОИ: {oi_growth_pct:.2f}% ({oi_source})")
...
async def main():
    ...
    try:
        await bot.send_message(chat_id=CHAT_ID, text="🤖 Бот «ПАРТИЗАН» (1H) успешно запущен на Bybit ОИ!")
    except Exception as e:
