...
# ============================================================
# 🏹 ПАРТИЗАН v6.1 — 1H SHELF & HYBRID BREAKOUT
# ============================================================
...
logger = logging.getLogger("PARТIZAN_v6.1")
...
async def fetch_klines(session, symbol):
    ...
    if not data:
        return []
    candles = [parse_kline(row) for row in data.get("data", [])]
...
# ============================================================
# SEARCH PIPELINE WITH DETAILED METRICS
# ============================================================
...
def find_best_shelf(candles):
    ...
    # Расширенный поиск: ищем окончания полок за последние SHELF_SEARCH_DEPTH свечей
...
def build_signal_message(symbol, ticker, candidate, rvol, oi, oi_growth):
    ...
    # Монета в моноширинном формате HTML <code> для кликабельного копирования
    clean_symbol = symbol.replace("-USDT", "")
    
    msg = (
        f"🏹 <b>ПАРТИЗАН v6.1 — ПЕРВЫЙ ИМПУЛЬС</b>\n\n"
...
async def evaluate_symbol(session, symbol, ticker):
    try:
        ...
        closed = candles[:-1] # Анализируем строго закрытые свечи
...
def log_stats(symbols_count, signal_count):
    logger.info(
        "📊 СКАН #%d | Пар=%d | 300 свечей=%d | Проверок полок=%d | "
...
async def scan_market(session):
    ...
    logger.info("🔎 Сканирование %d пар...", len(symbols))
...
async def health(request):
    return web.Response(text="ПАРТИЗАН v6.1 OK")
...
async def main():
    logger.info("🏹 Запуск ПАРТИЗАН v6.1...")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        welcome_text = (
            "🚀 <b>ПАРТИЗАН v6.1 запущен и готов к работе!</b>\n\n"
