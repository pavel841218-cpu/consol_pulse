...
MAX_DIST_FROM_EMA20_PCT = 2.5       # Макс отрыв от EMA20 (не более 2.5%)
MAX_PULLBACK_FROM_HIGH_PCT = 0.9     # Макс откат от хая 5M (не более 0.9%)
...
# ============================================================
#           ПРОВЕРКА 5M ИМПУЛЬСА (С ЗАЩИТОЙ ОТ ДАМПА)
# ============================================================
...
def check_5m_breakout(candles_5m, shelf):
    ...
    if body_pct < MIN_5M_BODY_PCT or body_pct > MAX_5M_BODY_PCT:
        return None

    # 5. ДИАПАЗОН
    range_pct = ((h - l) / l) * 100
    if range_pct < MIN_5M_RANGE_PCT or range_pct > MAX_5M_RANGE_PCT:
        return None
...
async def build_shelf_if_needed(session, symbol, semaphore):
    # ПРАВИЛЬНЫЙ КЭШ: Если полка уже есть, возвращаем её без спама в лог
...
async def main():
    ...
    try:
        await bot.send_message(
            chat_id=CHAT_ID,
            text=(
                "🤖 <b>ПАРТИЗАН v2 запущен (Fix Cache)</b>\n\n"
