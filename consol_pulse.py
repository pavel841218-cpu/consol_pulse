    ...
    async def load_hourly_klines(self, symbol: str) -> Tuple[Optional[List[Dict[str, Any]]], Optional[float]]:
        ...
        if symbol in self.kline_cache:
            ...
            if age < 55:
                ...
                return cached["klines"], cached["ema80"]  # возвращаем ema80
        
        try:
            # Для EMA80 достаточно 100 свечей, но оставим 205 для построения базы
            ...
            async with self.session.get(url, timeout=10) as resp:
                ...
                if len(klines) < 100:  # нужно минимум 80 закрытых свечей
                    return None, None
                
                # Исключаем последнюю незакрытую свечу
                closed = klines[-81:-1]  # 80 закрытых свечей
                closes = [float(x["close"]) for x in closed]
                last_closed_candle_time = int(closed[-1]["time"])
                
                # Инкрементальное обновление EMA80
    ...
    async def check_consolidation_and_pump(self, target_data: Dict[str, Any]) -> None:
        ...
        async with self.semaphore:
            ...
            for idx in range(len(history) - 1, -1, -1):
                ...
                # Ослабленный ATR-фильтр (было 0.6)
                if atr_base_pct >= 1.5:
                    break
        ...
        if "message" in data:
            ...
            if text == "/status":
                status_text = (
                    ...
                    f"• Base ATR: *< {escape_markdown('1.5%')}*\n"
