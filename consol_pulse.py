...
# ============================================================
#              ПАРТИЗАН — EMA / SHELF TEST v4.1 (BINGX BUGFIX)
# ============================================================
...
TIMEFRAME = "15m"
KLINE_LIMIT = 240  # 60 часов истории

# Расширили диапазон полки, чтобы видеть длинные флэты перед ракетой
SHELF_MIN_CANDLES = 6
...
# BREAKOUT CONFIG
MIN_BREAKOUT_PCT = 0.6
MAX_BREAKOUT_PCT = 8.0  # Увеличено, чтобы ловить быстрые ракеты в процессе 15m свечи

# LIQUIDITY CONFIG
MIN_24H_VOLUME_USDT = 800_000  # Базовый лимит честного суточного объема
RVOL_SPIKE_THRESHOLD = 3.5     # Если RVOL > 3.5, пробиваем фильтр объема!
...
async def get_session():
    ...
    if session is None or session.closed or now - session_created_at > SESSION_MAX_AGE:
        ...
        session = aiohttp.ClientSession(
            timeout=timeout, connector=connector, headers={"User-Agent": "PARTIZAN-EMA-TEST/4.1"}
        )
...
# ============================================================
# MAIN ANALYZER WITH ROLLING 24H VOLUME & RVOL BOOST
# ============================================================
...
async def analyze_symbol(symbol):
    try:
        ...
        # 1. Фильтр свежих листингов (минимум 150 свечей ~ 37 часов жизни)
        if len(candles) < 150:
            return None

        # 2. Честный автономный расчет Rolling 24h Volume по последним 96 свечам
        ...
        # 3. Фильтр объема: Либо объём >= $800k, ЛИБО идет аномальный всплеск (RVOL >= 3.5)
...
async def health(request):
    return web.Response(text=f"PARTIZAN EMA TEST v4.1 OK\nScans: {stats['scans']}\nSignals: {stats['signals']}\n")
...
async def main():
    logger.info("🚀 ПАРТИЗАН EMA TEST v4.1 (Запуск)")
    await start_server()
    while True:
        try:
