...
# ==========================================================
# LOGGING (уровень управляется переменной окружения LOG_LEVEL)
# ==========================================================
...
# ENV & CONFIG
# ==========================================================
BOT_NAME_RENDER: str = os.getenv("RENDER_SERVICE_NAME", "consol-pulse")
...
SELF_URL: str = os.getenv("RENDER_EXTERNAL_URL", f"http://localhost:{PORT}")
...
class ConsolidationMonitor:
    ...
    async def get_active_symbols(self) -> List[Dict[str, Any]]:
        """Получает список фьючерсных пар с фильтром по цене и чёрному списку (без фильтра суточного объёма)."""
        if not self.session or self.session.closed:
            return []
        ...
        try:
            url = f"{BINGX_API_URL}/tickers"
            async with self.session.get(url, timeout=15) as resp:
                if resp.status != 200:
                    logger.error(f"Get tickers error: status {resp.status}")
                    return self.active_symbols_cache
                data = await resp.json()
                if data.get("code") != 0:
                ...
                logger.info(f"Активных символов после фильтрации: {len(result)}")
                if not result:
                    logger.warning("Нет подходящих символов! Проверьте ценовой диапазон, blacklist и работу API BingX.")
                return result
        except Exception as e:
    ...
    async def check_consolidation_and_pump(self, target_data: Dict[str, Any]) -> None:
        ...
        async with self.semaphore:
            ...
            # 1. Фильтр тренда (EMA200)
            if current_close < ema200:
                self.increment_stat("filtered_by_trend")
                logger.debug(f"{symbol} отклонён: цена ниже EMA200 ({current_close} < {ema200})")
                return
            ...
            if current_volume < previous_volume * 1.3:
                self.increment_stat("filtered_by_volume")
                logger.debug(f"{symbol} отклонён: объём не вырос к предыдущей свече ({current_volume} < {previous_volume*1.3})")
                return
            ...
            if current_volume < peak_volume * 1.5:
                self.increment_stat("filtered_by_volume")
                logger.debug(f"{symbol} отклонён: объём {current_volume} < пиковый в базе *1.5 = {peak_volume*1.5}")
                return
            ...
            # === ИСПРАВЛЕННЫЙ БЛОК OI ===
            old_last_oi = None
            if symbol in self.oi_history and self.oi_history[symbol]:
                old_last_oi = self.oi_history[symbol][-1]
            ...
            if USE_OPEN_INTEREST:
                ...
                if fresh_oi is not None and len(history_oi) >= OI_MIN_HISTORY:
                    if old_last_oi is not None and fresh_oi <= old_last_oi:
                        self.increment_stat("filtered_by_oi")
                        logger.debug(f"{symbol} отклонён: OI не вырос (тек. {fresh_oi:.0f} <= пред. {old_last_oi:.0f})")
                        return

                    previous_oi_values = history_oi[:-1]
                    if len(previous_oi_values) > 0:
                        average_oi = sum(previous_oi_values) / len(previous_oi_values)
                        if average_oi > 0:
            ...
            logger.info(f"Сигнал отправлен: {clean}")
            try:
        ...
        while True:
            try:
                ...
                if self._iteration % 15 == 0:
                    ...
                    s = self.stats_daily
                    logger.info(
                        f"📊 Сводка за день: Проверено: {s['checked_symbols']}, "
                        f"Сигналов: {s['signals']}, "
                        f"Тренд: {s['filtered_by_trend']}, "
                        f"Объём: {s['filtered_by_volume']}, "
                        f"Пробой: {s['filtered_by_breakout']}, "
                        f"OI: {s['filtered_by_oi']}"
                    )

                symbols = await self.get_active_symbols()
                if not symbols:
                    logger.warning("Нет активных символов. Проверьте фильтры или API.")
                ...
                logger.exception(f"Глобальная ошибка в цикле: {e}")
            await asyncio.sleep(CHECK_INTERVAL)
