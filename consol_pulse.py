...
# ============================================================
# 🏹 ПАРТИЗАН v6.5.3 — FUNNEL LOGGING & REAL-TIME IMPULSE
# ============================================================
...
async def scan_market(session):
    ...
    # ВЫВОД СТАРОЙ АГРЕГИРОВАННОЙ ВОРОНКИ В КОНСОЛЬ
    logger.info(
        "📊 СКАН #%d | Пар=%d | Ок свечей=%d | Проверок полок=%d | Ширина=%d | "
        "EMA20=%d | EMA40=%d | EMA80=%d | Касания=%d | Без старого пампа=%d | Импульсы=%d | Цель +4%%=%d | СИГНАЛЫ=%d",
        scan_counter,
        len(symbols),
        stats["ok_candles"],
        stats["shelves_eval"],
...
async def health(request):
    return web.Response(text="ПАРТИЗАН v6.5.3 OK")

async def main():
    logger.info("🏹 Запуск ПАРТИЗАН v6.5.3 (Воронка возвращена)...")
    app = web.Application()
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=SESSION_TIMEOUT), connector=aiohttp.TCPConnector(limit=MAX_CONCURRENT_REQUESTS)) as session:
        await send_telegram(session, "🚀 <b>ПАРТИЗАН v6.5.3 запущен!</b>\nПолная воронка логирования восстановлена.")
        while True:
