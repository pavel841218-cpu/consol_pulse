...
# ============================================================
# 🏹 ПАРТИЗАН v6.5 — STRICT BULLISH & ANTI-MM FILTER
# ============================================================
...
# ИМПУЛЬС И ОБЪЕМ
BREAKOUT_TARGET_PCT = 4.0          # Твоя цель в 4%
...
MAX_UPPER_WICK_RATIO = 2.0         # Верхний фитиль не должен быть больше 2x от тела
MIN_BODY_PCT = 0.3                # Минимальное тело свечи пробоя (без доджи/сквизов)
MAX_VOL_TO_OI_RATIO = 3.0         # Защита от кручения объема ботами
...
logger = logging.getLogger("PARТIZAN_v6.5")
...
def is_market_maker_noise(breakout_candle, open_interest=None):
    """Отсекает алгоритмический шум, фитили и кручение объема."""
    ...
    # 1. Слишком малое тело свечи
    if body_pct < MIN_BODY_PCT:
        return True

    # 2. Огромный верхний фитиль (ММ сбросил ордера/сквизанул)
    if body > 0 and (upper_wick / body) > MAX_UPPER_WICK_RATIO:
        return True

    # 3. Накрутка объема роботами относительно OI
...
def find_best_shelf(candles):
    ...
    for end in range(total_closed - 1, min_end, -1):
        # Бычий тренд: EMA20 строго выше EMA80
...
async def evaluate_symbol(session, symbol, ticker):
    try:
        ...
        # ПРОВЕРКА НА МАРКЕТ-МЕЙКЕРАИ И СКВИЗЫ
...
async def health(request):
    return web.Response(text="ПАРТИЗАН v6.5 OK")

async def main():
    logger.info("🏹 Запуск ПАРТИЗАН v6.5...")
    app = web.Application()
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=SESSION_TIMEOUT), connector=aiohttp.TCPConnector(limit=MAX_CONCURRENT_REQUESTS)) as session:
        await send_telegram(session, "🚀 <b>ПАРТИЗАН v6.5 запущен!</b>\nАктивирован модуль Анти-ММ и трендовый веер EMA.")
        while True:
            start_time = time.time()
            try:
                await scan_market(session)
            except Exception as e:
                logger.exception("Ошибка сканирования: %s", e)
