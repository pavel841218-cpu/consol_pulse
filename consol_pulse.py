...
# Отслеживаемые монеты на 1m (Watchlist)
watched_coins = {} # { 'SOPH-USDT': added_timestamp }
last_signals = {}
    ...
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            ...
            for item in data.get("data", []):
                ...
                if not sym.endswith("-USDT") or any(x in sym for x in ["NCSK", "FOOTBALL", "INDEX", "_"]):
                    continue
...
def analyze_market_15m(candles):
    ...
    # 1. СБРОС ММ (SHORT): RVOL >= 2.8x (было 5x), Верхний фитиль > 38%
    if rvol >= 2.8 and wick_ratio >= 0.38 and last["close"] < last["open"]:
        return "MM_DISTRIBUTION", {"rvol": rvol, "wick": wick_ratio * 100, "price": last["close"]}

    # 2. ИМПУЛЬС (LONG): Бычий веер EMA 7 > 25 > 99, RVOL >= 1.3x, без гигантского фитиля
    is_fan = (e7 > e25) and (e25 > e99)
    dist_e7 = ((last["close"] - e7) / e7) * 100

    if is_fan and rvol >= 1.3 and dist_e7 <= 2.2 and wick_ratio < 0.30:
...
def get_watch_keyboard(symbol):
    ...
    kb.add(InlineKeyboardButton(text=f"👁 Следить за {symbol} (1m)", callback_data=f"watch_{symbol}"))
    return kb

# ====================== SCANNERS ======================

async def scan_watched_1m(session, bot):
    """Сверхбыстрый сканер 1m для монет на радаре"""
    while True:
        now = time.time()
        for symbol, added_time in list(watched_coins.items()):
            if now - added_time > 3600: # Авто-удаление через 1 час
            ...
            # Ловим микро-фитиль сброса (>40%) прямо на 1m
            if wick_ratio >= 0.40 and last["close"] < last["open"]:
                ...
                del watched_coins[symbol] # Снимаем с радара после сигнала

        await asyncio.sleep(5) # Сканируем каждые 5 сек

async def main_loop(session, bot):
    while True:
        try:
...
async def process_watch(callback_query: CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id, text=f"👁 {symbol} переведен на 1m радар!")
    await bot.send_message(CHAT_ID, f"🎯 <b>{symbol}</b> зафиксирован. Бот ищет микро-фитиль сброса на 1m свечах!")
...
async def on_startup(dp):
    ...
    # Web server для Render HealthCheck
    app = web.Application()
    ...
    # Фоновые процессы
    asyncio.create_task(main_loop(session, bot))
    asyncio.create_task(scan_watched_1m(session, bot))
