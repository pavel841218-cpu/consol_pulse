...
# Технические параметры стратегии (ИДЕАЛЬНАЯ ДИНАМИЧЕСКАЯ КРИПТО-НОЖКА ДЛЯ ДЕШЕВЫХ МОНЕТ)
MIN_PRICE = 0.0001
MAX_PRICE = 1.0               # 🔥 ВЕРНУЛИ СУЖЕННЫЙ ФИЛЬТР ЦЕНЫ СТРОГО ДО 1.0 USDT
MIN_24H_VOLUME = 500000

MIN_BOX_PERIOD = 12           # Минимальная длина прямоугольника (12 часов)
MAX_BOX_PERIOD = 24           # Максимальная длина прямоугольника (24 часа)
...
class ConsolidationMonitor:
    ...
    async def check_consolidation_and_pump(self, symbol: str):
        async with self.semaphore:
            url = f"{BYBIT_API_URL}/kline?category=spot&symbol={symbol}&interval=60&limit={MAX_BOX_PERIOD + 2}"
            try:
                async with self.session.get(url, timeout=10) as resp:
                    ...
                    if len(klines) < MIN_BOX_PERIOD + 2:
                        return
                    ...
                    # Перебор коробок от 24ч до 12ч
                    for current_box_len in range(MAX_BOX_PERIOD, MIN_BOX_PERIOD - 1, -1):
                        if len(klines) < current_box_len + 2:
                            continue

                        box_klines = klines[-(current_box_len + 2):-2]
                        ...
                        if current_volume_usdt > (avg_box_turnover * VOLUME_X_TRIGGER) and current_price > highest_box_price:
                            ...
                            msg = (
                                ...
                                f"📉 Длина полки: `{current_box_len} часов` 📦\n"
                            ...
                            logger.info(f"🔥 НАЙДЕНА КРИПТО-НОЖКА: {symbol} (Коробка: {current_box_len}ч)")
    ...
    async def start_loop(self):
        ...
        await self.send_telegram_signal("🚀 **Бот Крипто-ножки запущен! Мониторинг дешевых монет (< 1$). Полки от 12 до 24 часов.**")
...
async def handle_index(request):
    return web.Response(text="Бот Крипто-ножки работает и сканирует рынок дешевых монет! 🚀", status=200)
        ...
        if "message" in data and "text" in data["message"]:
            ...
            if text == "/status":
                status_msg = (
                    ...
                    f"📦 Диапазон полок: от `{MIN_BOX_PERIOD}` до `{MAX_BOX_PERIOD}` свечей\n"
                    f"📐 Плотность базы: до `{MAX_BOX_RANGE_PCT}%`\n"
...
async def keep_alive_ping():
    await asyncio.sleep(30) # Первичная пауза при запуске
    while True:
        try:
            # Дёргаем веб-сервер внешним HTTP-запросом каждые 4 минуты
        ...
        await asyncio.sleep(240) # Пауза в 4 минуты
...
async def main():
    ...
    # 🟢 Запуск задачи предотвращения сна в фоне
