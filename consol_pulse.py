...
# ============================================================
# 🏹 ПАРТИЗАН v6.2 — 1H SHELF (PRICE FILTER & FIX KLINE LIMIT)
# ============================================================
...
# Диапазон цен (только дешевые монеты / щиткоины)
...
logger = logging.getLogger("PARТIZAN_v6.2")
...
async def fetch_tickers(session):
    ...
    for row in rows:
        ...
        # Фильтр по диапазону цен (0.0001 - 1.0 USDT) и объему 24h
...
def find_best_shelf(candles):
    # Порог снижен до 100 свечей для надежности получения истории
    if len(candles) < 100:
        return None
...
def build_signal_message(symbol, ticker, candidate, rvol, oi, oi_growth):
    ...
    msg = (
        f"🏹 <b>ПАРТИЗАН v6.2 — ПЕРВЫЙ ИМПУЛЬС</b>\n\n"
...
def log_stats(symbols_count, signal_count):
    logger.info(
        "📊 СКАН #%d | Пар=%d | 100 свечей=%d | Проверок полок=%d | "
...
async def scan_market(session):
    ...
    # Отбираем только те монеты, которые прошли фильтр по цене (0.0001 - 1.0 USDT)
...
async def health(request):
    return web.Response(text="ПАРТИЗАН v6.2 OK")
...
async def main():
    logger.info("🏹 Запуск ПАРТИЗАН v6.2...")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        welcome_text = (
            "🚀 <b>ПАРТИЗАН v6.2 запущен и готов к работе!</b>\n\n"
